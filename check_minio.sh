...
DOCKERFILE_URL="https://github.com/${GITHUB_OWNER}/${GITHUB_REPO}/raw/master/Dockerfile.release"
...
function get_minio_version() {
    curl -s "https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/releases/latest" | jq -r '.tag_name'
}
...
function check_app_version() {
    ...
    if grep -q "ARG MINIO_VERSION=${APP_VERSION}" "${TMPDIR}/Dockerfile"; then
        echo "latest"
    fi
}

MINIO_VERSION=$(get_minio_version)
MC_VERSION=$(get_mc_version)

CHECK_LATEST_VERSION=$(check_app_version ${MINIO_VERSION})
...
ENV_VARS=("MINIO_VERSION" "MC_VERSION")
...
sed -i "s|MINIO_VERSION?=.*$|MINIO_VERSION?=${MINIO_VERSION}|" "${APP_DIR}/Makefile"
